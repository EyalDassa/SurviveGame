    ...
    private void initViews() {
        this.menu_BTN_start.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                String id = menu_EDT_id.getText().toString().trim();
                if (!isValidId(id)) {
                    menu_EDT_id.setError("ID must be exactly 9 digits");
                    menu_EDT_id.requestFocus();
                    return;
                }
                menu_EDT_id.setError(null);
        ...
        new Thread() {
            public void run() {
                ...
                if (data != null) {
                    ...
                    activity_Menu.startGame(Activity_Menu.this.menu_EDT_id.getText().toString(), data);
                }
            }
        }.start();
    }
